...
function bindAlternateDOMNode(current, alternate) {
  ...
  function normalizeChildren(
    /**@type {ReactNode | string | number | any} */ node,
  ) {
  ...
  while (currentQueue.length) {
    ...
    let currentChildrenArray = currentNode.children;
    let alternateChildrenArray = alternateNode.children;

    if (ReactNode.is(currentNode)) {
      currentChildrenArray = normalizeChildren(currentChildrenArray);
    }
    if (ReactNode.is(alternateNode)) {
      alternateChildrenArray = normalizeChildren(alternateChildrenArray);
    }

    if (
      Array.isArray(alternateChildrenArray) &&
      Array.isArray(currentChildrenArray)
    ) {
      // case (1)
