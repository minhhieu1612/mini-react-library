 ...
 */
function reconcileWorkLoop(root, isSync = true) {
  ...
  while (queue.length) {
    ...
    if (isSync) {
      ...
      // TODO: the present implementation is not correct, it needs to be handled
      // by scheduler module which has a timer to evaluate execution time and
      // support "yield to main thread" when the task exceeds permitted duration
      // normally 5ms. (1/3 a frame)
      Promise.resolve(performUnitOfWork(node));
    }
...
function bindAlternateDOMNode(current, alternate) {
  ...
  while (currentQueue.length) {
    ...
    const alternateChildrenArray = normalizeChildren(
      ReactNode.is(alternateNode) ? alternateNode.children : null,
    );
    ...
    if (alternateChildLength === currentChildLength) {
      ...
      alternateChildrenArray.forEach((alternateChild, index) => {
        ...
        if (index < currentChildLength) {
          currentQueue.push(currentChildrenArray[index]);
        } else {
          currentQueue.push(alternateChild);
          // add new node to current child
          if (!Array.isArray(currentNode.children)) currentNode.children = [];
          currentNode.children[index] = alternateChild;
        }
      });
    }
  }
 ...
 *   - remove: mark the node with "isRemoving" flag, so later dom
 *   node will be removed from dom tree
 */
function performUnitOfWork(/**@type {ReactNode} */ current) {
  ...
  switch (current.tag) {
    ...
    case REACT_TAGS.FUNCTION_COMPONENT: {
      ...
      if (current.children) {
        ...
        alternateChild = current.type(current.props);
...
function processDOM(node) {
        ...
        case "style": {
          if (typeof value === "object") {
            Object.keys(value).forEach((cssKey) => {
              if (/^--/.test(cssKey)) {
                element.style.setProperty(cssKey, value[cssKey]);
              } else {
                // camelCase to snake-case
                element.style.setProperty(
                  cssKey.replace(/([A-Z])/g, "-$1").toLowerCase(),
                  value[cssKey],
                );
              }
            });
  ...
  if (node.domUpdateParams) {
    if (node.domUpdateParams.isRemoving) {
      // remove
      // remove dom node
      const parentElement = node.domNode.parentElement;

      parentElement.removeChild(node.domNode);
      node.domNode = null;

      // dettach node from react node tree
      // then browser can free the node with garbage collection
...
function flushMutationEffectsSync() {
  if (!Shared.effectQueue.length) return;

  while (Shared.effectQueue.length) {
    const effect = Shared.effectQueue.shift();

    if (typeof effect.destroy === "function") {
      effect.destroy();
    }

    const destroy = effect.caller();
...
export const updateContainer = (node) => {
  ...
  reconcileWorkLoop(node);

  // should commit the update at the end of reconciler
  commitNodeSync(node);

  // flush all mutation effects
  flushMutationEffectsSync();
};
